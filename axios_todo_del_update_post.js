$(document).ready(function () {
    ...
    axios.get(userdata + "/todos?userId=1").then(function (post) {
      post.data.forEach((element) => {
        $("#row").append(
          '<div id="' +
            element.id +
            '" class="col-12 col-lg-4 col-md-4 col-sm-6 col-xl-3 mt-2 mb-5 " >' +
      ...
      $(".edit-user-class").click(function () {
        axios
          .get(userdata + "/todos/" + $(this).attr("user-id"))
          .then((edituser) => {
      ...
      $("#editForm").submit(function (event) {
        if (!$(this)[0].checkValidity()) {
          ...
          let data = {
            ...
            id: $("#edituserId").val(),
            title: $("#edittitle").val(),
            completed: $("#editbody").val(),
          };
          axios.patch(userdata + "/todos/" + data.id, data).then((res) => {
            let user = res.data;
            console.log(user);
      ...
      $("#deleteSubmit").click(function (e) {
        e.preventDefault();
        axios.delete(userdata + "/posts/" + $("#delete-user-id").val())
          .then((singleUser) => {
      ...
      $('#RegForm').submit(function (event) {
        if (!$(this)[0].checkValidity()) {
          ...
          let data = {
            ...
            title: $("#regtitle").val(),
            completed: $("#regbody").val()
          };
          axios.post(userdata + "/todos", data).then((res) => {
            let user = res.data;
            console.log(user);
